...
<template>
    <div id="register" class="border border-black border-primary rounded-4">
        <div class="m-5 d-flex flex-column">
            <form id="formularioRegistro" @submit.prevent="register()" class="form-group d-flex flex-column">
                ...
                <input placeholder="Usuario" @input="isValidUser()" type="text" required :class="inputClass"
                ...
                <input placeholder="password" type="password" required :class=inputClass @input="isValidPwd()"
                    v-model="user.pwd">
                <input placeholder="passsword repeat" required type="password" v-model="pwd_repeat"
                    @input="isValidPwd()" :class=inputClass>
            ...
            <router-link class="mx-auto btn btn-white m-3 border border-black h6 " to="/">
    ...
    <div class="modal" v-if="alert">
        <modal>
            <div class="content">
                <h2>{{alertMessage }}</h2>
                <button @click="alert = false" class="btn btn-primary m-5">Cerrar</button>
            </div>
        </modal>
    </div>
